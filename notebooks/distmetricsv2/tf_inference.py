...
def _estimate_params_via_streamed_patches(
    ...
    # If model is provided, try to detect its device
    ...
    if acq_dts_float is None:
        # Create dummy dates if none provided (for backward compatibility)
        acq_dts_float = [0.0] * len(imgs_copol)
    acq_dts_tensor = torch.tensor([acq_dts_float], dtype=torch_dtype, device=device)  # Shape: (1, T)
    ...
    for patch_batch, slices in tqdm(
        ...
        chip_mean, chip_logvar = model(patch_batch, acq_dts_tensor)
